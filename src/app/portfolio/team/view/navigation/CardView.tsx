...
export default function CardView({ member, index, shouldShowInfo, onClick }: CardViewProps) {
  ...
  // Slide dès qu'il y a plus d'un rôle
  const canSlide = member.roles.length > 1;

  // Animation automatique et continue
  useEffect(() => {
    if (!canSlide) {
      setTranslateX(0);
      return;
    ...
    const speed = 30; // px per second (plus lent pour être plus agréable)
    let totalWidth = 0;
  ...
  }, [canSlide]); // Plus de dépendance à hovered
    ...
    >
      ...
      <div className={styles.body}>
        ...
        <div className={styles.rolesWrap} ref={containerRef}>
          {!canSlide ? (
            // Static view: show first 2 roles + +N badge if needed
            <div className={styles.rolesStatic}>
              ...
              {member.roles.length > 2 && (
                <span className={styles.moreTag}>+{member.roles.length - 2}</span>
              )}
            </div>
          ) : (
            // Sliding marquee view en continu
